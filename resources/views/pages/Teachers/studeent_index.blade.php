                    ...
                    <td>
                        ...
                        <label class="block text-gray-500 font-semibold sm:border-r sm:pr-4">
                            <input name="attendences[{{ $student->id }}]"
                                @if (isset(
                                    $student->attendance()->where('attendence_date', date('Y-m-d'))->first()->student_id)) {{ $student->attendance()->first()->attendence_status == 1 ? 'checked' : '' }} @endif
                        ...
                        <label class="ml-4 block text-gray-500 font-semibold">
                            <input name="attendences[{{ $student->id }}]"
                                @if (isset(
                                    $student->attendance()->where('attendence_date', date('Y-m-d'))->first()->student_id)) {{ $student->attendance()->first()->attendence_status == 0 ? 'checked' : '' }} @endif
